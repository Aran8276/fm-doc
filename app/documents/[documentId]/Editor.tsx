...
import { compileMdxToServer } from "./actions";
...
export default function Editor({
  ...
  const [compiledHtml, setCompiledHtml] = useState("<div></div>");
  ...
  useEffect(() => {
    startTransition(async () => {
      try {
        const data = await compileMdxToServer(debouncedContent);
        if (data.html) {
          setCompiledHtml(
            `<div>
              <h1>${title}</h1>
        ...
        console.error("Failed to fetch compiled MDX", error);
        setCompiledHtml(
  ...
  return (
    <>
      <div className="absolute opacity-25 hover:opacity-100 transition-all duration-700 flex items-center justify-center w-6 h-6 right-12 bottom-12 z-[100]">
        <div
      ...
      <div className="absolute p-8 transition-all hover:opacity-100 duration-700 opacity-25 right-0 z-[100] top-16">
      ...
      <ResizablePanelGroup className="fixed" direction="horizontal">
        ...
        <ResizablePanel collapsible ref={rightPanelRef}>
          <div
            className="p-8 prose h-screen overflow-y-scroll pb-24"
